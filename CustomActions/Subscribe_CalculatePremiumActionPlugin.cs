...
{
    public class Subscribe_CalculatePremiumActionPlugin : IPlugin
    {
                ...
                try
                {
                    ...
                    if (Product == "Professional Indemnity")
                    {
                        ...
                        if (subsQuote.Attributes.Contains("lux_quoteoption1"))
                        {
                            ...
                            foreach (var item1 in tradeList.Entities.GroupBy(x => x.GetAttributeValue<EntityReference>("lux_subscribequoteoption")))
                            {
                                var limit = 0M;
                                foreach (var item in item1)
                                {
                                    ...
                                    subscribepisectionpremium["lux_subscribeprofessionalindemnityquote"] = new EntityReference("lux_subscribepiquote", subsQuote.Id);
                                    organizationService.Create(subscribepisectionpremium);
                                }

                                Entity subscribesectionpremium1 = new Entity("lux_subscribepisectionpremium");
                                subscribesectionpremium1["lux_section"] = new OptionSetValue(972970006);
                                subscribesectionpremium1["lux_ratingfigures"] = new Money(limit);
                                subscribesectionpremium1["transactioncurrencyid"] = new EntityReference("transactioncurrency", subsQuote.GetAttributeValue<EntityReference>("transactioncurrencyid").Id);
                                subscribesectionpremium1["lux_subscribequoteoption"] = new EntityReference("lux_subscribequoteoption", item1.FirstOrDefault().GetAttributeValue<EntityReference>("lux_subscribequoteoption").Id);
                                subscribesectionpremium1["lux_subscribeprofessionalindemnityquote"] = new EntityReference("lux_subscribepiquote", subsQuote.Id);
                                organizationService.Create(subscribesectionpremium1);
                            }
