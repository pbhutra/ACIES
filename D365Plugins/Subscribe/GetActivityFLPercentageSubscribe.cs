...
{
    public class GetActivityFLPercentageSubscribe : IPlugin
    {
            ...
            if (context.InputParameters.Contains("Target") && context.Depth == 1)
            {
                ...
                try
                {
                    ...
                    var tradefetch = $@"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                                          <entity name='lux_subscribeactivitiesfl'>
                                            <attribute name='lux_name' />
                                            <attribute name='createdon' />
                                            <attribute name='lux_percentage' />
                                            <attribute name='lux_detailsofservice' />
                                            <attribute name='lux_subscribeactivitiesflid' />
                        ...
                        tradefetch = $@"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                                                  <entity name='lux_subscribeactivitiesfl'>
                                                    ...
                                                    <filter type='and'>
                                                      <condition attribute='statecode' operator='eq' value='0' />
                                                      <condition attribute='lux_subscribeprofessionalindemnityquote' operator='eq' uiname='' uitype='lux_subscribepiquote' value='{cpeQuote.Id}' />
                                                      <condition attribute='lux_subscribeactivitiesflid' operator='ne' uiname='' uitype='lux_subscribeactivitiesfl' value='{entity.Id}' />
                                                    </filter>
                                                  </entity>
                                                </fetch>";
                    }
                    ...
                    if (tradeList.Entities.Count > 0)
                    {
                        ...
                        {
                            list.Add(item.Attributes.Contains("lux_detailsofservice") ? item.FormattedValues["lux_detailsofservice"] : "");
                        }
                        cpeQuote["lux_othersoftware"] = String.Join(",", list.ToArray());
                    }
                    ...
                    cpeQuote["lux_activitiesfltotalpercentage"] = TotalPercentage;
