...
{
    public class CalculateGrossPremiumRecruitment : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.InputParameters.Contains("Target") && context.InputParameters["Target"] is Entity && context.Depth == 1)
            {
                ...
                try
                {
                    ...
                    var PremiumRow = organizationService.Retrieve("lux_specialistschemerecruitmentpremuim", entity.Id, new ColumnSet(true));
                    var recruitmentQuote = organizationService.Retrieve("lux_recruitmentquotes", PremiumRow.GetAttributeValue<EntityReference>("lux_recruitmentquote").Id, new ColumnSet(true));
